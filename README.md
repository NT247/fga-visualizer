...
Permission tester — Select a subject (human user or AI agent), a resource type, and an action to see whether access is granted or denied, with the full inheritance chain and evaluation steps shown. Includes agent-specific output showing required token scope, audit log entry, and why traditional MFA does not apply to agents.
Why agent auth is included
WorkOS has identified agent identity as a core frontier: who are your agents and what are they allowed to do. FGA supports AI agent subjects natively, and the permission tester explicitly shows how human and agent access diverge, which directly maps to the infrastructure WorkOS is building.
How to run
...
This is a prototype built for the WorkOS PM role application. Not an official WorkOS product. Schema output is illustrative and based on the WorkOS FGA documentation at workos.com/docs/fga.
